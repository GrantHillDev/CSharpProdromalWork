namespace BankEncapsulation
{
    public class Program
    {
        ...
        {//this is an example of how we're creating a new instance of the BankAccount class -- var Account = new BankAccount()// the BankAccount() represents a constructor (so, this is one way to write constructors outside of the class which they're based off of).
            ...
            Console.WriteLine("");
            var amountToDeposit1 = double.Parse(Console.ReadLine());//double.TryParse(Console.ReadLine());for some reason, .TryParse didn't work.... some error regarding method overloading?
            Console.WriteLine("");//Remember, any input that's taken in by a Console.ReadLine is going to come in the form of a string.
            Account.Deposit(amountToDeposit1);//we defined what the program should do when taking input in the form of a string from a user, by way of storing it within the variable we created above called, amountToDeposit. More flexible and effective way of taking user input rather than just utilizing the Console.ReadLine method by itself.
            Console.WriteLine("");//what's happening here is very simple, but its mechanics might be confusing. Account.Deposit is the account variable we created using the Deposit method we also defined within its custom BankAccount class which again grants us access to the otherwise private balance field we also defined within the same class, and whether it's because of syntax or otherwise, we set the value of our private balance field equal to a new custom variable called 'amount' within the Deposit method, and the amount variable is going to work with the amountToDeposit variable to not only take in a user's input when prompted for one, but also ensure that it's in the desired format (double type).
            ...
            Console.WriteLine("");
            Console.WriteLine("Seems like you suddenly wish to make yet another contribution to that same bank account you just made a deposit to -- how much will it be this time?");
            Console.WriteLine("");
            var amountToDeposit2 = double.Parse(Console.ReadLine());
            Console.WriteLine("");
            Account.Deposit(amountToDeposit2);
            Console.WriteLine("");
            Console.WriteLine($"Confirmed; your bank account balance now has a total of ${Account.GetBalance()}.");
            Console.WriteLine("");
            Console.WriteLine("Whoops, it appears as though you feel as if you may have deposited way too much into that account of yours -- better take some back out!");
            Console.WriteLine("");
